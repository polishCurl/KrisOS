...
KrisOS_task_static_template(secondTimer, 256, 3)
KrisOS_task_static_template(ledPWM, 256, 41)
...
int main(void) {	
	...
	// Initialise the prime numbers task
	KrisOS_task_create_static(&primesTask, primes, &primesStack[primesStackSize], 
		primesPriority); 
	
	// Initialise the second counter task
	KrisOS_task_create_static(&secondTimerTask, secondTimer, 
		&secondTimerStack[secondTimerStackSize], secondTimerPriority); 
	
	// Initialise the RGB PWM LED task
	KrisOS_task_create_static(&ledPWMTask, ledPWM, 
		&ledPWMStack[ledPWMStackSize], ledPWMPriority); 
