...
void UART0_Handler(void) {
	...
	if (UART0->RIS & (1 << TXRIS_Pos)) { 
		UART0->ICR |= (1 << TXIC_Pos);
		uart_software_to_hardware_fifo();
	...
	if (UART0->RIS & (1 << RXRIS_Pos)) {
		UART0->ICR |= (1 << RXIC_Pos);
	...
	if (UART0->RIS & (1 << RTRIS_Pos)) {
		UART0->ICR |= (1 << RTIC_Pos);
		uart_hardware_to_software_fifo();
	}
}
...
void uart_hardware_to_software_fifo(void) {
	uint8_t character;
	while ((UART0->FR & (1 << FR_RXFE_Pos)) == 0 && !uart_rx_fifo_is_full()) {
		character = UART0->DR;
...
void uart_software_to_hardware_fifo(void) {
	uint8_t character;
	while ((UART0->FR & (1 << FR_TXFF_Pos)) == 0 && !uart_tx_fifo_is_empty()) {
