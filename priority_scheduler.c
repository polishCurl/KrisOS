...
* Brief:    	Preemptive priority scheduler with task aging to avoid starvation
...
void run_scheduler(void) {
	...
	runPtr->lastRun = TICKS;
	
	runPtr->next = runPtr;
	for (i = 0; i < readyQueue.noOfTasks; i++)
		if (readyQueue.tasks[i]->counter < runPtr->counter && runPtr->next->lastRun > readyQueue.tasks[i]->lastRun)
			runPtr->next = readyQueue.tasks[i];
		
	if (runPtr->next != runPtr) {
		runPtr->counter = runPtr->priority * TIME_SLICE_MUL_PRIORITY;
		runPtr->next->counter = 0;
	}
}
...
void update_counters(void) {
	...
	// Perform READY tasks aging. Decrement the counter of each READY task to gradually
	// increase its priority so that task starvation is avoided
	for (i = 1; i < readyQueue.noOfTasks; i++)
		readyQueue.tasks[i]->counter--;
		
	// Fix the counter of the task currently RUNNING. 
	runPtr->counter++;
...
uint32_t create_task(void* startAddr, int32_t id, uint32_t priority, 
	...
	readyQueue.tasks[readyQueue.noOfTasks]->counter = priority * TIME_SLICE_MUL_PRIORITY;
	readyQueue.tasks[readyQueue.noOfTasks]->lastRun = 0;
...
uint32_t resume_task(int32_t toResumeID) {
	...
	if (i != NOT_FOUND) {
		...
		// Update both queues
		taskToResume = remove_from_queue(i, &blockedQueue);
		add_to_queue(taskToResume, &readyQueue);
		
		// Set the counter to dynamic priority starting value of the task
		taskToResume->counter = taskToResume->priority * TIME_SLICE_MUL_PRIORITY;
